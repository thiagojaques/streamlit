...
if uploaded_file is not None and processar:
    try:
        ...
        fig_previsao, ax = plt.subplots(figsize=(10,5)) #cria um gráfico com um conjunto de eixos
        ax = ts_data.plot(ax=ax) #atribui ao aixo os dados da série temporal <--colocando os dados originais e a previsão no mesmo gráfico
        previsao.plot(ax=ax, style='r--') #esse comando pega a previsão e plota com este eixo <-- está colorindo a previsão para diferenciar.

        col1, col2, col3, = st.columns([3,3,2]) #cria colunas e referência o tamanho de cada coluna
        with col1:
            st.write('Decomposição') # figura da decomposição dos dados
            st.pyplot(fig_decomposicao)

        with col2:
            st.write('Previsão') # figura da previsão dos dados
